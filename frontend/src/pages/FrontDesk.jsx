...
import { mockVisitors, mockCompanies, mockBuildings } from '../mockData';
...
const FrontDesk = () => {
  ...
  const [manualCompanionsDetails, setManualCompanionsDetails] = useState([]);
  ...
  return (
    <div className="min-h-screen bg-secondary">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl md:text-4xl font-bold text-graphite mb-2">
            Portaria - {mockBuildings.find(b => b.id === user?.buildingId)?.name || 'Edifício'}
          </h1>
          <p className="text-base md:text-xl text-neutral-dark">Interface simplificada para controle de acesso</p>
        </div>
        ...
        {activeTab === 'history' && (
          <div>
            ...
            {visitors.filter(v => v.status !== 'pending').length > visibleCount && (
              <div className="mt-6 text-center">
                ...
                >
                  Carregar Mais ({visitors.filter(v => v.status !== 'pending').length - visibleCount} restantes)
                </Button>
              </div>
            )}
          </div>
